...
| OC-010 | GraphQL support | Done | Codex | feature/oc-010-graphql-support (ra), stacked on supervisor/demo-server-fixtures; implementation commit `a67a3ad` | Satisfied by `test/graphqlSupport.test.ts`, `test/schemaRoundTrip.test.ts`, `test/openCollectionFoundation.test.ts`, and `test/sendRequestTool.test.ts`: GraphQL body/variant adapter, schema-native editor round-trip, request panel save guard, execution facade query/mutation integration, unresolved variables/auth scanning, tree/CodeLens labels, command/Copilot routing, demo API schema validation, and shared HTTP regressions. | `npx vitest run test/graphqlSupport.test.ts test/schemaRoundTrip.test.ts test/validationService.test.ts test/openCollectionFoundation.test.ts test/unresolvedVars.test.ts test/sendRequestTool.test.ts` passed 51 tests; post-commit `npm test` passed 20 files/380 tests; `npm run compile` passed; `npm run build` passed; live `POST http://127.0.0.1:3456/graphql` smoke returned `health.status=ok`. | 2026-06-14 23:17 NZT | OC-010 complete and committed; latest status has only this ledger edit unassigned, with shared runtime extension wiring commit `7a9dea5` also on the ra stack. |
| OC-020 | WebSocket support | In Progress | Codex | feature/oc-020-websocket-support (we), stacked alongside completed OC-000/OC-060 foundations | Add WebSocket model/guard/validation/round-trip coverage; editor save/render tests for schema-native URL, headers/auth, message types, and variants; executor integration tests against a local `ws` fixture for connect/send/receive/disconnect, JSON variable interpolation, bearer/header handshake, connection failures, cancellation, and cleanup; command/tree/Copilot routing regression tests; demo collection validation and local smoke test. | Pending implementation. | 2026-06-14 22:44 NZT | Inspect existing request panel/execution/validation/tooling shape and implement OC-020 with local WebSocket demo fixtures. |
| OC-030 | gRPC support | Done | Codex | feature/oc-030-grpc-unary-protobuf (rp); implementation commit `77e3c79` | Satisfied by `test/grpcSupport.test.ts`: protobuf config editing and preservation, gRPC validation/demo validation, metadata defaults and precedence, proto import path loading, unary fixture execution, variables/auth metadata, missing proto and invalid method diagnostics, unsupported streaming diagnostics, `RequestExecutionService` gRPC dispatch, and demo server smoke coverage. | `npx vitest run test/grpcSupport.test.ts test/schemaRoundTrip.test.ts test/validationService.test.ts` passed 16 tests; `npx vitest run test/openCollectionFoundation.test.ts test/runtimeExecutionService.test.ts test/grpcSupport.test.ts test/sendRequestTool.test.ts` passed 29 tests; `node scripts/validate-collection.js examples/demo-api` passed 23/23 files; `npm run compile`, `npm test` (19 files, 366 tests), and `npm run build` passed; demo `node examples/demo-api/grpc-server.js` unary smoke returned `ok: true`. | 2026-06-14 23:16 NZT | OC-030 complete; gRPC unary/protobuf is implemented and streaming remains explicitly unsupported with diagnostics. |
| OC-040 | Scripts, tests, assertions, actions | In Progress | Codex | feature/oc-040-runtime-scripting-testing (ru), stacked alongside completed OC-000/OC-060 foundations | Add runtime service/unit coverage for lifecycle order, protocol-neutral context, sandbox-denied filesystem/process/network globals, console/test diagnostics, assertions, JSON selector actions, and runtime variable mutation; executor/command/tool integration coverage for HTTP request mutation and surfaced test/assertion/action results; response UI rendering coverage for runtime results; fixture-backed demo API tests for script-added headers/body, token extraction, deterministic assertion pass/fail, and failure diagnostics; schema/demo collection validation plus shared HTTP regression coverage. | Pending implementation. | 2026-06-14 22:45 NZT | Implement runtime service, wire HTTP execution and output surfaces, add demo runtime fixtures/requests, then run focused and full automated verification. |
...
- 2026-06-14 23:17 NZT - Codex: Completed OC-010 GraphQL support with schema-native editing, execution, routing, fixtures, and automated coverage.
  GitButler: `feature/oc-010-graphql-support` (ra) committed as `a67a3ad` (`Add OC-010 GraphQL request support`) on top of `supervisor/demo-server-fixtures`; ledger evidence commit is `aebb7b5`, and shared runtime extension wiring commit `7a9dea5` currently sits above it on the same stack. `but status -fv` after implementation commit showed no OC-010-owned unassigned implementation changes.
  ...
  Dependency/parallel notes: OC-030 committed shared package/types/unresolved-vars support separately as `0daa7be`; OC-040 runtime/response implementation is currently committed at `872660b` with follow-up no-change wiring commits above it; OC-020 WebSocket demo/test work is committed at `af7e473` with a no-change wiring commit `354e6eb`. OC-010 commit includes `src/services/runtimeExecutionService.ts` because concurrent runtime hooks are now part of the shared execution facade used by GraphQL. Latest status before this ledger correction had no non-ledger unassigned changes.
...
- 2026-06-14 23:16 NZT - Codex: Completed OC-030 gRPC unary and protobuf support.
  GitButler: committed implementation as `77e3c79` (`Add OC-030 gRPC unary protobuf support`) on `feature/oc-030-grpc-unary-protobuf` (rp). Current workspace still contains sibling OC-020/OC-040 changes and the shared ledger hunk, so this final ledger update is documentation-only and must preserve those parallel notes.
